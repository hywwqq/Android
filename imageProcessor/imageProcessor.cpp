...
static void _S_shades(QString sourceFile, QString destFile)
{
    QImage image(sourceFile);
    int width = image.width();
    int height = image.height();
    ...
    while(i < width)
    {
        for(int k = 0; k < 10; k++)
        {
            for(int j = 0; j < height; j++)
            {
                image.setPixel(i+k, j, qRgb(255, 255, 255));
            }
        }
        i+=50;
    }
...
static void _H_shades(QString sourceFile, QString destFile)
{
    QImage image(sourceFile);
    int width = image.width();
    int height = image.height();
    ...
    while(j < height)
    {
        for(int k = 0; k < 10; k++)
        {
            for(int i = 0; i < width; i++)
            {
                image.setPixel(i, j+k, qRgb(255, 255, 255));
            }
        }
        j+=50;
    }
